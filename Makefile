...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend personal_automations db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend personal_automations db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend personal_automations db upgrade

db-downgrade:
	uv run --directory backend personal_automations db downgrade

db-current:
	uv run --directory backend personal_automations db current

db-history:
	uv run --directory backend personal_automations db history

# === Server ===
run:
	uv run --directory backend personal_automations server run --reload

run-prod:
	uv run --directory backend personal_automations server run --host 0.0.0.0 --port 8000
...
# === Taskiq ===
taskiq-worker:
	uv run --directory backend personal_automations taskiq worker

taskiq-scheduler:
	uv run --directory backend personal_automations taskiq scheduler
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/personal_automations"
...
help:
	@echo ""
	@echo "personal_automations - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
